...
function planJourney() {
    ...
    if (originData.line === destinationData.line) {
        ...
        result += `<p>Take <strong>${originData.buses[1]}</strong> to Majestic (Seats: ${originData.seats[1]}) and then transfer to <strong>${destinationData.buses[0]}</strong> (Seats: ${destinationData.seats[0]}).</p>`;
    }

    result += `<p>Arrival at Origin: <strong>${originData.timings[0]}</strong></p>`;
    result += `<p>Departure from Destination: <strong>${destinationData.timings[1]}</strong></p>`;
